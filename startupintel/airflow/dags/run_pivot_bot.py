"""Airflow DAG for PivotBot - Pivot detection."""
...
def run_pivot_bot_batch(**context) -> dict:
    """Run PivotBot for all active startups."""
    import os
    import sys
    ...
    async def _run_batch():
        ...
        try:
            ...
            bot = PivotBot(db, neo4j, redis, rag, llm)
            results = []
            ...
            for startup_id in startup_ids[:10]:
                try:
                    ...
                    results.append({
                        ...
                        "score": result.score,
                        "pivot_confidence": result.signal_breakdown.get("pivot_confidence", 0),
                        "status": "completed",
                    })
...
with DAG(
    ...
    description="Run PivotBot weekly for pivot detection",
    schedule_interval="0 9 * * 2",  # Weekly on Tuesday at 9 AM
    start_date=days_ago(1),
    catchup=False,
    tags=["bots", "pivot", "strategy"],
) as dag:

    run_batch = PythonOperator(
        task_id="run_pivot_bot_batch",
        python_callable=run_pivot_bot_batch,
    )
