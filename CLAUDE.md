...
- FastAPI application with `/health` and demo RunwayBot stress scoring.
- Pydantic settings wired to `.env`.
- Async SQLAlchemy PostgreSQL models for the core schema.
- Redis, Neo4j, and Postgres client helpers.
- BaseBot orchestration and RunwayBot scoring/event emission.
- Initial tests for API routes and RunwayBot behavior.

Treat the remaining bots, ingestion connectors, Kafka workers, Airflow DAG bodies, Slack integration, and full RAG/LLM implementations as planned work unless a concrete file already exists.
